...
**A scene swap loads before it paints.** `PlaybackController.replaceScene` used to repaint synchronously "for a no-flash swap of what's already warm", then load and repaint. That holds only when the swap introduces nothing new; when it does, the first paint is a frame missing its media, held for the whole fetch — and now that reaching for an unloaded asset throws, it is the difference between working and not. There is one paint now, after the load, and the canvas keeps the previous complete frame until then. `replaceScene` still returns its index synchronously; await `whenReplaced()` (or `FrameHandle.whenReplaced()`) before screenshotting or reading the tree. `PlaybackController.screenshot()` is likewise `async` and loads first.
