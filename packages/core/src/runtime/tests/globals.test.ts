...
describe("LayerStack – scene filtering", () => {
    ...
    it("rejects a node handed in where a factory belongs", () => {
        // A layer built once is shared between runtimes; the first to dispose
        // leaves the next with a hollow tree. Saying so at construction beats a
        // layer that silently stops painting on the second mount.
        expect(() => new LayerStack("overlay", [new Rect({ width: 10, height: 10 })], VIEWPORT))
            .toThrow(/factory, not a node/);
    });

    it("builds a fresh tree per stack, so two runtimes never share one", () => {
        const factory = () => new Rect({ width: 10, height: 10 });
        const first = new LayerStack("overlay", [factory], VIEWPORT);
        first.dispose();
        // The second stack runs the factory again rather than adopting the first
        // stack's (now disposed) node, so its layer is intact.
        const second = new LayerStack("overlay", [factory], VIEWPORT);
        expect(second.isEmpty).toBe(false);
        expect(() => second.layout(BOUNDS, new FakeMeasurer())).not.toThrow();
    });
