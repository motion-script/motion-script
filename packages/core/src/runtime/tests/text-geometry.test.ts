...
import { TextBlockLayout, type TextBlockSource } from '@/render/text-layout';
 ...
 */
class BlockScope extends FakeMeasurer implements TextBlockSource {
    constructor(private readonly block: TextBlockLayout | null = FOUR_CHARS) {
        super();
    }
    layoutTextBlock(): TextBlockLayout | null {
        return this.block;
    }
}
...
function place(node: { layout: (r: BoxBounds, s: Measurer2D) => void }, rect: BoxBounds): void {
