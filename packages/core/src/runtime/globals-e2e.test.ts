...
describe("global layers – playback", () => {
    ...
    it("advances layers on the project clock, so they don't restart at a cut", () => {
        const log: string[] = [];
        const probe = new Probe(log, "bg");
        const scenes = [probeScene(log, "intro", 4), probeScene(log, "outro", 4)];
        const globals = new ProjectGlobals({ backgrounds: [probe] }, VIEWPORT);
        const precomp = new Precomp(scenes, VIEWPORT, FPS, catalog(), scope, { globals }).run();
        const evaluator = new StateEvaluator(
            scenes, VIEWPORT, FPS, catalog(), precomp.scenes.map(s => s.frameCount), scope, globals,
        );
        ...
        expect(probe.clock.time).toBeCloseTo(5 / FPS, 5);
    });
});
...
describe("global layers – node reuse", () => {
    it("adopts a config-provided node again after a controller is disposed", () => {
        const log: string[] = [];
        const probe = new Probe(log, "fg");
        const scenes = [probeScene(log, "a", 3)];
        const mount = () => {
            const globals = new ProjectGlobals({ overlays: [probe] }, VIEWPORT);
            const precomp = new Precomp(scenes, VIEWPORT, FPS, catalog(), scope, { globals }).run();
    ...
    it("does not leave a layer attached to a disposed frame", () => {
        const provided = new Rect({ width: 10, height: 10 });
        const globals = new ProjectGlobals({ backgrounds: [provided] }, VIEWPORT);
        globals.dispose();
