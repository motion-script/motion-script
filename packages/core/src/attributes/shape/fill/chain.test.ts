...
describe('Fills builders', () => {
    ...
    it('image produces an image fill with common options merged', () => {
        expect([...Fills.image('./bg.jpg', { blend: 'overlay', opacity: 0.2 })]).toEqual([
...
describe('FillChain', () => {
    it('appends fills in order while staying immutable', () => {
        const base = Fills.image('./bg.jpg', { opacity: 0.2 });
    ...
    it('matches the example from the request', () => {
        const chain = Fills.image('./background.jpg', { blend: 'overlay', opacity: 0.2 })
