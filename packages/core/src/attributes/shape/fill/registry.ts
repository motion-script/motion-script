...
import { dotGridFill } from "./implementations/dot-grid";
import { gridFill } from "./implementations/grid";
...
import { canvas3DFill } from "./implementations/canvas3d";
...
const FILLS = new Map<string, FillData<FillResolved>>([
    ...
    ["dotGrid", dotGridFill as FillData<FillResolved>],
    ["grid", gridFill as FillData<FillResolved>],
    ...
    ["canvas3D", canvas3DFill as FillData<FillResolved>],
]);
...
export function resolveFill(prop: FillProp): FillResolved {
    ...
    if (prop instanceof Scene3D) return resolveFill({ type: "canvas3D", scene: prop });
