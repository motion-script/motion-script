...
describe('ImageFilters builders', () => {
    it('blur stores radius under value', () => {
        expect([...ImageFilters.blur(6)]).toEqual([{ type: 'blur', value: 6 }]);
    });

    it('alpha and exposure carry their value', () => {
        expect([...ImageFilters.alpha(0.5)]).toEqual([{ type: 'alpha', value: 0.5 }]);
        expect([...ImageFilters.exposure(2)]).toEqual([{ type: 'exposure', value: 2 }]);
    });
    ...
    it('curves carries points and optional channel', () => {
        expect([...ImageFilters.curves([[0, 0], [1, 1]], 'r')]).toEqual([
