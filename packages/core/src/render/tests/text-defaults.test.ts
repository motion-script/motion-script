...
import { AssetTracker, CanvasAssetTracker } from '@/assets/tracker';
...
import { declareLayoutAssets } from '@/nodes/node/node-walk';
...
describe('asset discovery sees the inherited family', () => {
    ...
    it('declares the font a drawn op will inherit, so it actually loads', () => {
        ...
        const tracker = new CanvasAssetTracker(new ManifestAssetCatalog(manifest));
        tracker.start(0);
        declareLayoutAssets(root, tracker);
        tracker.end();
