...
import { CameraScope, Node2D, NodeConfig, Node2DProps } from "@/nodes/2d/node2d";
import { property } from "@/attributes/properties/decorator";
import { anchorProperty, fillProperty } from "@/attributes/properties/typed";

/** @internal */
export interface Canvas2DProps extends Node2DProps {
    /**
 ...
 *
 * A `Canvas2D` is a plain {@link Node2D} that doubles as the scene's layout frame
 ...
 */
/** @internal */
export class Canvas2D extends Node2D<Canvas2DProps> implements FlowHost {
    ...
    // engine so Rect and Canvas2D don't each carry a copy.
    private readonly _flowLayout = new FlowLayout(this);

    constructor(props: NodeConfig<Canvas2D, Canvas2DProps>) {
        super(props);
    ...
    zoomTo(zoom: number, duration: number, ease?: EasingFunction): Command<Canvas2DProps> {
        return this.to({ zoom } as Partial<Canvas2DProps>, duration, ease);
    }
    ...
    panTo(lookAt: Vector2, duration: number, ease?: EasingFunction): Command<Canvas2DProps> {
        return this.to({ lookAt } as Partial<Canvas2DProps>, duration, ease);
    }
    ...
    headingTo(heading: number, duration: number, ease?: EasingFunction): Command<Canvas2DProps> {
        return this.to({ heading } as Partial<Canvas2DProps>, duration, ease);
    }
    ...
    fillTo(to: Fill, duration: number, options?: TweenOptions<FillResolved[]>): Command<Canvas2DProps> {
        ...
        return this.delayed(
            options?.delay,
            this.command<Canvas2DProps>(t => ({ fill: lerp(from, target, t) }) as Partial<Canvas2DProps>, duration, options?.ease),
        );
    }
    ...
    overlayTo(to: Fill, duration: number, options?: TweenOptions<FillResolved[]>): Command<Canvas2DProps> {
        ...
        return this.delayed(
            options?.delay,
            this.command<Canvas2DProps>(t => ({ overlay: lerp(from, target, t) }) as Partial<Canvas2DProps>, duration, options?.ease),
        );
    }

    /** Prefix `animation` with `delay` seconds of holding still (see `ShapeNode.delayed`). */
    private delayed(delay: number | undefined, animation: Command<Canvas2DProps>): Command<Canvas2DProps> {
        if (!delay) return animation;
        const hold = this.command<Canvas2DProps>(() => ({}), delay);
        return commandSequence<Canvas2DProps>(this, hold, animation);
    }
