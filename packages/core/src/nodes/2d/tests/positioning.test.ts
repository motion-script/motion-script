...
/**
 * A fixed-size leaf that exposes the box its parent laid it into. `layoutBounds` is
...
function layoutStage(root: Canvas2D, scope: FakeMeasurer): void {
...
describe('positioning — resolution', () => {
    it('defaults to relative, and a root has no parent to be absolute against', () => {
        ...
        const root = new Canvas2D({ width: 'fill', height: 'fill', children: [child] });
...
describe('positioning — absolute children leave the flow', () => {
    ...
    it('does not contribute to a freeform hug size', () => {
        ...
        const root = new Canvas2D({ width: 'fill', height: 'fill', children: [box] });

        layoutStage(root, scope);

        expect((box as any).layoutBounds.width).toBe(100);
        expect((box as any).layoutBounds.height).toBe(100);
    });
...
describe('positioning — absolute children are placed against the canvas', () => {
    it('cancels the offset of a nested parent, so x/y read as scene coordinates', () => {
        ...
        // A hug column pushed to the left half of the canvas: its own centre is
        // nowhere near the canvas centre, which is exactly what has to be undone.
        ...
        // The pinned tile's own x/y are 0, so it lands on the canvas centre no
        // matter where `inner` ended up.
        expect(pinned.global.x).toBeCloseTo(0, 6);
        expect(pinned.global.y).toBeCloseTo(0, 6);
    });

    it("the child's own x/y then offset from the canvas origin", () => {
        const scope = new FakeMeasurer();
        ...
        const root = new Canvas2D({ width: 'fill', height: 'fill', children: [inner] });
    ...
    it('measures against the canvas, so width="fill" fills the scene rather than the parent', () => {
        const scope = new FakeMeasurer();
    ...
    it('a childPositioning="absolute" container pins every child it holds', () => {
        ...
        const root = new Canvas2D({ width: 'fill', height: 'fill', children: [canvas] });
    ...
    it('works through a plain Node2D container (the default layout path)', () => {
        ...
        holder.add(pinned);
        const root = new Canvas2D({ width: 'fill', height: 'fill', children: [holder] });
