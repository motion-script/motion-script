 ...
 * `layoutBounds` is protected on Node2D, so a test subclass is the supported seam
...
describe('Row', () => {
    it('hugs its children along both axes by default', () => {
        ...
        row.add([new Tile(100, 50), new Tile(100, 80), new Tile(100, 50)]);
...
describe('Column', () => {
    it('hugs its children: width = widest child, height = sum + gaps', () => {
        ...
        col.add([new Tile(60, 100), new Tile(90, 100), new Tile(60, 100)]);
...
describe('Row/Column – default main-axis size when a direct child is fill', () => {
    ...
    it('promoted Row participates as a real fill sibling in an outer hug Row', () => {
        ...
        expect((innerRow as any).layoutBounds.width).toBe(1920 - 300);
    });
});
