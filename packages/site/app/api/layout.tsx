...
import { getApiPackages, getApiHomeSlug } from '@/lib/api'
import { VERSIONS } from '@/lib/versions'
import VersionedApiSidebar, {
  type ApiPackagesByVersion,
  type ApiHomeByVersion,
} from '@/components/docs/VersionedApiSidebar'
...
export default function ApiLayout({ children }: { children: React.ReactNode }) {
  // Pre-render every version's packages + API home; VersionedApiSidebar picks
  // one by pathname.
  const packages: ApiPackagesByVersion = Object.fromEntries(
    VERSIONS.map((version) => [version.version, getApiPackages(version)]),
  )
  const homes: ApiHomeByVersion = Object.fromEntries(
    VERSIONS.map((version) => [version.version, '/api/' + getApiHomeSlug(version).join('/')]),
  )
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center gap-3 sm:gap-6 px-4 h-14">
          <MobileSidebar>
            <VersionedApiSidebar packages={packages} homes={homes} />
          </MobileSidebar>
          ...
          <div className="hidden sm:flex items-center gap-1 text-sm">
            <Link href="/docs/intro" className="px-3 py-1.5 rounded-md text-muted-foreground hover:text-foreground transition-colors">
              Docs
            </Link>
            <Link href="/api/core" className="px-3 py-1.5 rounded-md text-foreground font-medium bg-foreground/5">
              API
            </Link>
