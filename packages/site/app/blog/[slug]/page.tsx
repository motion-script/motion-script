...
import { getAllBlogSlugs, getPost } from '@/lib/blog'
...
export async function generateMetadata({ params }: Props) {
  ...
  if (!post) return {}
  return {
    title: `${post.meta.title} — MotionScript`,
...
export default async function BlogPostPage({ params }: Props) {
  ...
  // Strip {/* truncate */} markers before compiling
  const cleanContent = post.content.replace(/\{\/\* truncate \*\/\}/g, '')

  const code = await compile(cleanContent, {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:py-16">
        ...
        <Link
          href="/blog"
          className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-10 no-underline hover:no-underline"
        >
          ← All posts
        </Link>
        ...
        <header className="mb-10">
          <h1 className="text-3xl font-semibold text-foreground mb-4">{post.meta.title}</h1>
          ...
          {post.meta.authors.length > 0 && (
            <div className="flex flex-wrap gap-4 pt-4 border-t border-border">
              {post.meta.authors.map((author) => (
                <div key={author.name} className="flex items-center gap-2.5">
                  {author.image_url && (
                    <Image
