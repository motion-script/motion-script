...
### One tween per command

Every `to()` and every helper above returns a **single** command. Sequence them
with `sequence(...)` and run them together with `parallel(...)` — the same two
combinators everything else in a scene composes through.
