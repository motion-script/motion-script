...
Two caveats. `u_time` is **node age, not scene time** — it starts when the node appears, which is what makes it identical whether you played to a frame or seeked to it. And `u_scale` folds in the pixel ratio *and* the camera zoom, so a 4× export sees a different value from a 1× one: use it for a hairline that should stay one pixel wide, not for anything that decides the look.
