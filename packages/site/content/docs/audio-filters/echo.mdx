...
const clip = new Sound({ src: './voice.mp3', filters: AudioFilters.echo(0.3, 0.4) });

// With explicit mix — mostly dry, subtle echo
const clip = new Sound({ src: './voice.mp3', filters: AudioFilters.echo(0.5, 0.6, 0.25) });
...
const clip = new Sound({ src: './narration.mp3', filters: AudioFilters.echo(0.4, 0.5, 0) });
clip.start();

// Crossfade to a wet echo over 3 seconds
yield* tween(3, (t) => {
  clip.set({ filters: AudioFilters.echo(0.4, 0.5, lerpNumber(0, 0.6, easeOut(t))) });
});
```
...
AudioFilters.echo(0.3, 0.5).lowpass(3000)

// Quieter source into a long, washy reverb-like echo
AudioFilters.gain(0.7).echo(0.6, 0.75, 0.4)
```
