...
const clip = new Sound({ src: './piano.mp3', filters: AudioFilters.tremolo(5, 0.6) });
...
const clip = new Sound({ src: './pad.mp3', filters: AudioFilters.tremolo(1, 0) });
clip.start();

// Intensify the wobble over 4 seconds
yield* tween(4, (t) => {
  clip.set({ filters: AudioFilters.tremolo(lerpNumber(1, 8, t), lerpNumber(0, 0.9, t)) });
});
```
