...
- `name`: the project name, used for output filenames
...
:::tip The `?scene` suffix
Each scene file `export default createScene(...)`; the `?scene` import hands you
a ready-to-use scene instance with a stable identity stamped on it, so you list
`myScene` directly rather than constructing anything yourself.
:::
...
(one instance per file, so each keeps its own identity):
...
Render your project with `ms screenshot first` and you'll see your blue square sitting in the middle of the frame:
