...
Here's the trick: a `Rect` isn't just a shape, it's also a container. Give it a `flow` prop and it lines up its children automatically.

## Stack things in a row or column

Set `flow="horizontal"` to place children side by side, or `flow="vertical"` to stack them top to bottom. Add `gap` for spacing and `padding` for breathing room around the edges.
...
| `flow` | What it does |
|---|---|
| `'horizontal'` | children side by side, left to right |
| `'vertical'` | children stacked, top to bottom |
| `'freeform'` | children layered on top of each other |
...
```tsx
<Rect flow="vertical" gap={16} padding={32} fill="#1e293b" cornerRadius={16} align="center">
  <Rect width={80} height={80} fill="royalblue" cornerRadius={12} />
  <Text text="Motion Script" fontSize={40} fontWeight={700} fill="white" />
</Rect>
```
...
export default createScene(function* (stage) {
  ...
  stage.add(
    <Rect ref={card} flow="vertical" gap={16} padding={32} fill="#1e293b" cornerRadius={16}>
      <Rect width={80} height={80} fill="royalblue" cornerRadius={12} />
      <Text text="Motion Script" fontSize={40} fontWeight={700} fill="white" />
    </Rect>
  );
