...
The [Graphics2D API](/docs/custom-node/graphics/overview) has its own `effects()` call for filtering part of a drawn figure rather than a whole node. It takes exactly the same values. See [Transforms and effects](/docs/custom-node/graphics/transforms#effects).
