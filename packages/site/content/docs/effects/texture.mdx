...
<Rect effects={Effects.texture({ src: './paper.jpg' })} />
...
export default createScene(function* (stage) {
  ...
  stage.add(<Rect ref={card} effects={Effects.texture({ src: './paper.jpg', amount: 0 })} />);

  yield* card().to({ effects: Effects.texture({ src: './paper.jpg', amount: 1 }) }, 1, easeInOut('quad'));
});
```
