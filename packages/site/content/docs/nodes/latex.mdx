...
import { LatexCanvas } from '@site/src/components/docs/node-scenes';

# LaTeX

The `Latex` node renders mathematical equations using MathJax.

<LatexCanvas />

`Latex` ships as a registry component, not a package dependency: `ms add`
copies its source straight into your project, so you can read and edit it
...
```tsx
stage.add(
  <Rect flow="vertical" gap={20} padding={32} fill="#1e293b" cornerRadius={16}>
