...
```tsx
import { createScene, Ellipse } from 'motion-script';
...
| `ratio` | `number` | `1` | Inner radius as a fraction of the outer, so the size of the hole. `0` is a solid disk, `0.5` a donut. At the default `1` the hole reaches the outer edge, which fills as a solid ellipse again |
...
```tsx
import { createScene, Ellipse, createRef, easeInOut } from 'motion-script';
