...
```tsx
import { createScene, Polygram } from 'motion-script';
...
```tsx
import { createScene, Polygram, createRef, easeInOut } from 'motion-script';
...
- The star is inscribed within the node's bounding rect, and outer vertices touch the rect edges.
