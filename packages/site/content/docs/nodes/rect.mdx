...
export default createScene(function* (stage) {
  ...
  stage.add(
    <Rect flow="horizontal" gap={48} padding={44} cornerRadius={24}>
...
|---|---|---|---|
| `flow` | `'horizontal' \| 'vertical' \| 'freeform'` | `'freeform'` | Layout mode for children |
...
If a direct child is `'fill'` on the axis `Rect` would otherwise hug (its `flow`'s main axis — width for `horizontal`, height for `vertical`, both independently for `freeform`), that axis defaults to `'fill'` instead, so the child has real space to fill into. See [Size](../attributes/size#default-sizing) for the full rule.
...
All props animate with `.to()`. Transition `flow` to cross-fade between layout modes, or animate `gap` to expand/collapse spacing:
