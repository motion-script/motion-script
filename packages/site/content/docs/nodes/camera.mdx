...
```tsx
import { createScene, Camera, Rect } from 'motion-script';
...
| `lookAt` | `{ x: number, y: number }` | `{ x: 0, y: 0 }` | World-space point that maps to the center of the viewport — what the camera is aimed at. Same word as `Graphics3D.perspective({ lookAt })` |
...
```tsx
import { createScene, Camera, Rect, createRef, easeInOut } from 'motion-script';
...
export default createScene(function* (stage) {
  ...
  yield* cam().to({ lookAt: { x: 240, y: 0 } }, 1.2, easeInOut);

  // Zoom in on it
  yield* cam().to({ zoom: 1.8 }, 0.8, easeInOut);

  // Pull back home
  yield* cam().to({ lookAt: { x: 0, y: 0 }, zoom: 1 }, 1.2, easeInOut);
});
```
...
- `lookAt` is in world-space coordinates relative to the children's own origin, not screen coordinates.
