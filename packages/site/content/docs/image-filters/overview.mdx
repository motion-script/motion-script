...
Image filters are pixel-level operations applied to `Image` nodes and `image` fill types via the `filters` prop. Unlike [Node Effects](../effects/blur) (which apply after the entire node is composited), image filters operate directly on the image texture before it is drawn.
