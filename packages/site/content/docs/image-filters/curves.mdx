...
filters={ImageFilters.curves({ points: [[0, 0.1], [0.5, 0.6], [1, 1]], channel: 'b' })}
...
```tsx
ImageFilters
  .curves({ points: [[0, 0], [0.5, 0.55], [1, 1]], channel: 'r' })   // warm reds
  .curves({ points: [[0, 0.05], [0.5, 0.5], [1, 0.95]], channel: 'b' }) // cool darks
```
