...
A `prop={() => ...}` binding re-evaluates when the **signals** it reads change. `clock.elapsed` is a plain field, not a signal, so this computes once and freezes:

```tsx
{/* Wrong. Runs once, then never again. No error, just a still image. */}
<Box3D rotation={() => [0, this.clock.elapsed * 90, 0]} />
```

Two correct options:

* Drive the motion from a tweened signal. This also puts it on the timeline, so it scrubs.
* Read `this.clock.elapsed` inside `renderSelf()` or `buildScene3D()`, which do re-run every frame.
