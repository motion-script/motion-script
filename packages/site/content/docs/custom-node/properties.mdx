...
| `@paddingProperty` | `Padding` | `0` | Scalar, symmetric or per edge. Lerps per edge |
| `@alignProperty` | `Alignment` | `'center'` | A named position, resolved to a per-axis vector. `'left'` to `'right'` slides |
| `@pivotProperty` | `Alignment` | `{ x: 0, y: 0 }` | Like `@alignProperty`, but with the pivot convention that `Node.pivot` uses |
