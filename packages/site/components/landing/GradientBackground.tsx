...
interface Plane { positions: Float32Array; uvs: Float32Array; indices: Uint16Array }

function createPlane(width: number, height: number, wSeg: number, hSeg: number): Plane {
  const wHalf = width / 2; const hHalf = height / 2
  const gridX1 = wSeg + 1; const gridY1 = hSeg + 1
  const segW = width / wSeg; const segH = height / hSeg
...
function hexToLinear(hex: string): Float32Array {
  ...
  return new Float32Array([srgbToLinear(((n >> 16) & 255) / 255), srgbToLinear(((n >> 8) & 255) / 255), srgbToLinear((n & 255) / 255)])
}
...
const gradientVert = `precision highp float;attribute vec3 position;attribute vec2 uv;uniform mat4 projectionMatrix;uniform mat4 modelViewMatrix;varying vec2 vUv;varying float vElevation;uniform float uTime;${SNOISE}void main(){vUv=uv;vec3 pos=position;float noise=snoise(vec3(pos.x*1.5,pos.y*1.5,uTime))*0.4;noise+=snoise(vec3(pos.x*3.0,pos.y*3.0,uTime*0.5))*0.15;pos.z+=noise;vElevation=noise;gl_Position=projectionMatrix*modelViewMatrix*vec4(pos,1.0);}`
const gradientFrag = `precision highp float;uniform float uTime;uniform vec3 uColor1;uniform vec3 uColor2;uniform vec3 uColor3;uniform vec3 uColor4;varying vec2 vUv;varying float vElevation;void main(){float t1=sin(uTime*0.5+vUv.x*3.14159)*0.5+0.5;float t2=cos(uTime*0.3+vUv.y*3.14159)*0.5+0.5;vec3 color1=mix(uColor1,uColor2,t1);vec3 color2=mix(uColor3,uColor4,t2);vec3 finalColor=mix(color1,color2,vUv.y+vElevation*0.5);float brightness=0.85+vElevation*0.3;finalColor*=brightness;gl_FragColor=vec4(finalColor,1.0);}`
const gridVert = `precision highp float;attribute vec3 position;attribute vec2 uv;uniform mat4 projectionMatrix;uniform mat4 modelViewMatrix;varying vec2 vUv;varying vec3 vPosition;uniform float uTime;void main(){vUv=uv;vPosition=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`
const gridFrag = `#extension GL_OES_standard_derivatives : enable\nprecision highp float;uniform float uGridSize;uniform float uLineWidth;uniform float uTime;uniform vec3 uColor;varying vec2 vUv;varying vec3 vPosition;void main(){vec2 animatedPos=vPosition.xy;animatedPos.y-=uTime;vec2 grid=abs(fract(animatedPos/uGridSize-0.5)-0.5)/fwidth(animatedPos/uGridSize);float line=min(grid.x,grid.y);float lineStrength=1.0-min(line,1.0);float distFromCenter=length(vPosition.xy)/10.0;float fade=1.0-smoothstep(0.3,1.0,distFromCenter);float alpha=lineStrength*fade*0.2;if(alpha<0.01)discard;gl_FragColor=vec4(uColor,alpha);}`
const particlesVert = `precision highp float;attribute vec3 position;uniform mat4 projectionMatrix;uniform mat4 modelViewMatrix;uniform float uSize;uniform float uScale;void main(){vec4 mvPosition=modelViewMatrix*vec4(position,1.0);gl_Position=projectionMatrix*mvPosition;gl_PointSize=uSize*(uScale/-mvPosition.z);}`
const particlesFrag = `precision highp float;uniform vec3 uColor;uniform float uOpacity;void main(){gl_FragColor=vec4(uColor,uOpacity);}`
...
function buildProgram(gl: WebGLRenderingContext, vsrc: string, fsrc: string, attribs: string[]): WebGLProgram {
...
function makeBuffer(gl: WebGLRenderingContext, target: number, data: Float32Array | Uint16Array): WebGLBuffer {
...
const GRADIENT_COLORS = { c1: hexToLinear('#6366f1'), c2: hexToLinear('#8b5cf6'), c3: hexToLinear('#06b6d4'), c4: hexToLinear('#ec4899') }
...
function GradientCanvas() {
  ...
  useEffect(() => { gridColorRef.current = hexToLinear(isDark ? '#d1d5db' : '#6b7280') }, [isDark])

  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const gl = (canvas.getContext('webgl', { alpha: true, antialias: true, premultipliedAlpha: true, depth: true }) ||
    ...
    const gradU = { proj: gl.getUniformLocation(progGradient, 'projectionMatrix'), mv: gl.getUniformLocation(progGradient, 'modelViewMatrix'), time: gl.getUniformLocation(progGradient, 'uTime'), c1: gl.getUniformLocation(progGradient, 'uColor1'), c2: gl.getUniformLocation(progGradient, 'uColor2'), c3: gl.getUniformLocation(progGradient, 'uColor3'), c4: gl.getUniformLocation(progGradient, 'uColor4') }
    const gridU = { proj: gl.getUniformLocation(progGrid, 'projectionMatrix'), mv: gl.getUniformLocation(progGrid, 'modelViewMatrix'), time: gl.getUniformLocation(progGrid, 'uTime'), gridSize: gl.getUniformLocation(progGrid, 'uGridSize'), lineWidth: gl.getUniformLocation(progGrid, 'uLineWidth'), color: gl.getUniformLocation(progGrid, 'uColor') }
    const partU = { proj: gl.getUniformLocation(progParticles, 'projectionMatrix'), mv: gl.getUniformLocation(progParticles, 'modelViewMatrix'), size: gl.getUniformLocation(progParticles, 'uSize'), scale: gl.getUniformLocation(progParticles, 'uScale'), color: gl.getUniformLocation(progParticles, 'uColor'), opacity: gl.getUniformLocation(progParticles, 'uOpacity') }
    ...
    const frame = (now: number) => {
      ...
      gl.drawElements(gl.TRIANGLES, gradientPlane.indices.length, gl.UNSIGNED_SHORT, 0)
      ...
      gl.drawElements(gl.TRIANGLES, gridPlane.indices.length, gl.UNSIGNED_SHORT, 0)

      const mvParticles = multiply(view, multiply(rotationX(elapsed * 0.01), rotationY(elapsed * 0.02)))
