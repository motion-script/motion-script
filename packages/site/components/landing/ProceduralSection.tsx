  ...
  return (
    <section className="relative px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
      <div className="mx-auto w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        <CardShadow tilt className="relative rounded-2xl border border-border bg-[#17171C] overflow-hidden">
          <div className="flex items-center gap-1.5 px-4 py-3 border-b border-border">
            <span className="w-3 h-3 rounded-full bg-red-400/70" />
            <span className="w-3 h-3 rounded-full bg-yellow-400/70" />
            <span className="w-3 h-3 rounded-full bg-green-400/70" />
            <span className="ml-3 text-xs text-muted-foreground font-mono">scene.ts</span>
...
function CopyButton({ code }: { code: string }) {
  ...
  return (
    <button
      ...
      className="ml-auto inline-flex items-center gap-1.5 rounded-md border border-border bg-foreground/5 px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-foreground/10 hover:text-foreground"
    >
