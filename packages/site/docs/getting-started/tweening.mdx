...
export class MyScene extends Scene {
  *build() {
    ...
    this.add(<Rect ref={box} width={200} height={200} fill="royalblue" cornerRadius={12} />);
...
- `this.add(node)` — add a node or nodes to the scene
...
export class MyScene extends Scene {
  *build() {
    const items = ['Alpha', 'Beta', 'Gamma'].map((label, i) => {
      const item = new Rect({ width: 300, height: 60, fill: '#1e293b', cornerRadius: 8, opacity: 0 });
      this.add(item);
      return item;
    });
