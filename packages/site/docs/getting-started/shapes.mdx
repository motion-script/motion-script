...
| `cornerStyle` | `'rounded' \| 'angled'` (+ shorthand) | `'rounded'` | Rounded (arc) or angled (chamfer) corners |
...
### Corner radius and style

`cornerRadius` and `cornerStyle` share one shorthand grammar: a single
value applies to every corner, `{ topLeft, topRight, bottomLeft, bottomRight }` targets
corners individually, and `{ top, bottom }` / `{ left, right }` target pairs.
...
export class MyScene extends Scene {
  *build() {
    ...
    this.add(
      <Polygram ref={star} sides={5} ratio={0.4} width={300} height={300} fill="gold" cornerRadius={10} />
    );
...
| `cornerRadius` | `number` | `0` | Vertex rounding (outer points) |
| `cornerStyle` | `'rounded' \| 'angled'` | `'rounded'` | Vertex shape (arc vs chamfer) |
...
export class MyScene extends Scene {
  *build() {
    this.add(
      <Image src="./photo.jpg" fit="fill" width={800} height={450} cornerRadius={20} />
    );
...
| `fit` | `'fill' \| 'fit' \| 'tile' \| 'stretch'` | `'fill'` | How the image fills the node (`'fill'` = cover + crop, centered) |
| `cornerRadius` | `number \| CornerRadiusProps` | `0` | Corner rounding |
