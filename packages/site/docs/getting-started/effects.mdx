...
Chunks the node into rectangular blocks, like After Effects' Mosaic. The value is a **block count** — how many blocks span the node — so a count equal to the node's pixel size is pristine and lower counts are chunkier. Pass a `number` (uniform), `{ blocks, sharpColors? }`, or `{ horizontalBlocks, verticalBlocks, sharpColors? }`.

```tsx
<Rect
  width={300} height={300}
  fill={{ type: 'image', src: './logo.png', mode: 'fit' }}
  effects={FX.pixelate(60)}
/>
```

### Pixel-reveal animation

Start with a low block count (coarse) and animate up toward the node's resolution:
...
export class MyScene extends Scene {
  *build() {
    ...
    yield* tween(1.5, (t) => {
      logo.set({ effects: FX.pixelate(lerpNumber(40, 300, easeInOut(t))) });
    });
  }
}
```
...
| `{ type: 'pixelate', horizontalBlocks: number, verticalBlocks: number, sharpColors: boolean }` | Number of blocks per axis (AE Mosaic); `sharpColors` — solid blocks vs. blended |
