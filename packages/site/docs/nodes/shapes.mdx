...
| Prop | Type | Default |
|---|---|---|
| `cornerRadius` | `number \| { topLeft, topRight, bottomLeft, bottomRight } \| { top, bottom } \| { left, right }` | `0` |
| `cornerStyle` | `'rounded' \| 'angled'` (or the same per-corner / per-axis shorthand) | `'rounded'` |
...
Each corner can be a circular **rounded** arc or an **angled** chamfer (a straight diagonal cut). `cornerStyle` picks between them.

```tsx
{/* A chamfered (cut) corner */}
<Rect width={200} height={200} fill="tomato" cornerRadius={24} cornerStyle="angled" />
```

`cornerRadius` and `cornerStyle` share the same shorthand grammar: a single value applies to every corner, `{ topLeft, topRight, bottomLeft, bottomRight }` targets corners individually, and `{ top, bottom }` / `{ left, right }` target pairs.

```tsx
{/* Mixed: rounded top corners, angled bottom corners */}
<Rect
  width={240}
  height={160}
  fill="seagreen"
...
```tsx
<Polygon sides={6} width={200} height={200} fill="teal" cornerRadius={8} />
```

| Prop | Type | Default | Description |
|---|---|---|---|
| `sides` | `number` | `5` | Number of sides |
| `cornerRadius` | `number` | `0` | Vertex rounding |
| `cornerStyle` | `'rounded' \| 'angled'` | `'rounded'` | Vertex shape (arc vs chamfer) |

Polygons and polygrams apply a single `cornerStyle` to every vertex (their vertices don't map to named corners), so it takes a scalar rather than the per-corner shorthand.
...
```tsx
<Polygram sides={5} ratio={0.4} width={240} height={240} fill="gold" cornerRadius={10} />
```
...
| `cornerRadius` | `number` | `0` | Vertex rounding (outer points only) |
| `cornerStyle` | `'rounded' \| 'angled'` | `'rounded'` | Vertex shape (arc vs chamfer) |
...
```tsx
<Image src="./photo.jpg" fit="fill" width={800} height={450} cornerRadius={12} />
```
...
| `cornerRadius` | `number \| CornerRadiusProps` | `0` | Corner rounding |
