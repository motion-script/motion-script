import { createScene, createRef, Grid, Rect, Ellipse, Polygon, Polygram, Path, Line, Text, parallel, sequence, wait, easeInOut, easeOut } from "motion-script";
...
export default createScene(function* (stage) {
    ...
    const row = (children: any) => (
      <Rect flow="horizontal" gap={12} align={{ x: 0, y: 0 }}>
        {children}
      </Rect>
    );

    const cell = (label: string, children: any) => (
      <Rect flow="vertical" gap={12} fill={CARD} cornerRadius={16} padding={24} align={{ x: 0, y: 0 }}>
        {row(children)}
