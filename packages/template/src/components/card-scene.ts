...
export class CardStage extends ThreeStage {
    private data: GalleryItem[];

    // Engine settings
    private readonly PLANE_GAP: number = 8;
    private readonly PLANE_WIDTH: number = 3.2;
    private readonly PLANE_HEIGHT: number = 4.5;
    private readonly CAMERA_START_Z: number = 6;

    // Arrays to hold specific mesh references
    private imagePlanes: THREE.Mesh[] = [];
    private textPlanes: THREE.Mesh[] = [];
    ...
    private createAbstractTexture(data: GalleryItem): THREE.CanvasTexture {
        ...
        ctx.beginPath();
        ctx.moveTo(60, 200); ctx.lineTo(1024 - 60, 200);
        ctx.moveTo(60, 1440 - 200); ctx.lineTo(1024 - 60, 1440 - 200);
        ctx.stroke();
    ...
    private createTextTexture(data: GalleryItem, textOnLeft: boolean): THREE.CanvasTexture {
        const canvas = document.createElement('canvas');
        canvas.width = 1024;
        canvas.height = 1024;
        const ctx = canvas.getContext('2d')!;
        ...
        const align: CanvasTextAlign = textOnLeft ? 'right' : 'left';
