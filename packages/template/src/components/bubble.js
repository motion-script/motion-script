...
export class BubbleStage extends ThreeStage {
    ...
    onTime(t) {
        ...
        for (let i = 0; i < positions.count; i++) {
            vertex.fromBufferAttribute(this.originalPositions, i);
            normal.copy(vertex).normalize();
            const noise = this.simplex.noise3d(vertex.x * this.noiseScale + timeOffset, vertex.y * this.noiseScale + timeOffset, vertex.z * this.noiseScale + timeOffset);
