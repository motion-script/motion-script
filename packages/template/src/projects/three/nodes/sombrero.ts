import {
    Geo, Graphics3D, Mat, property, View3D,
    type Color, type NodeConfig, type NormalizedColor, type View3DProps,
} from "motion-script";
import { lerpColor, orbitOf, orbitPosition, resolveColor, snapFlag } from "./attributes";

export interface SombreroProps extends View3DProps {
 ...
 * Extends {@link View3D} rather than `Node`, so it inherits the whole bridge into
 * the 3D renderer and only has to override {@link view3D} — the one seam both the
 ...
 */
export class Sombrero extends View3D<SombreroProps> {
    ...
    constructor(props?: NodeConfig<Sombrero, SombreroProps>) {
        super(props as NodeConfig<View3D<SombreroProps>, SombreroProps>);
    }
    ...
    private addShell(g3: Graphics3D, range: number): void {
        ...
        g3.group({ key: "shell" }, (inner) => inner
            .box({
                ...box,
                unlit: true,
