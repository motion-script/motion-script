...
export class TextFillsScene extends Scene {
    ...
    *build() {
        ...
        yield* textRef().to({ fill: Fills.linearGradient(['#6990DD', '#E8617C'], { start: { x: -1, y: -1 }, end: { x: 1, y: 1 } }) } as any, dur, easeInOutQuad);
        // linear -> conic gradient
        yield* textRef().to({ fill: Fills.conicGradient(['#E8617C', '#F5C26B', '#6990DD', '#E8617C'], { startAngle: 0 }) } as any, dur, easeInOutQuad);
