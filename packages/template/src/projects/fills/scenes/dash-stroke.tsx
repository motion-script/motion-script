import { createScene, Fills, sequence, easeInOut, linear } from "@motion-script/core";
...
export default createScene(function* (stage) {
    ...
    const sample = strokeCard(stage, 'Dash Stroke', {
        stroke: { weight: 4, fill: Fills.color('#6990DD'), dash: [6, 10], dashOffset: 0, align: 'center' },
    });

    yield* sequence(
        // weight: hairline -> thick band.
        sample().strokeTo({ weight: 28, fill: Fills.color('#6990DD'), dash: [6, 10], align: 'center' }, 1.2, { ease: easeInOut('quad') }),
        // dash pattern: tight dots -> long dashes / wide gaps (snaps at midpoint).
        sample().strokeTo({ weight: 28, fill: Fills.color('#E8617C'), dash: [60, 40], align: 'center' }, 1.2, { ease: easeInOut('quad') }),
