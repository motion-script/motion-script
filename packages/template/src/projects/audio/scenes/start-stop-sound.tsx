...
import { createScene, createRef, Text, Rect, AudioFilters, easeInOut, easeOut, wait } from "motion-script";
...
export default createScene(function* (stage) {
    ...
    stage.add(
        <Rect width={'fill'} height={'fill'} group={'column'} gap={40}>
            <Text fontFamily={'Pixelify Sans'} text={'startSound / stopSound'} fontSize={96} fill={'gray'} width={'fill'} textAlign={'start'} />
    ...
    yield* dot().to({ scale: 1, fill: 'primary' } as any, 1.2, easeInOut('quad'));

    // Stop the sound explicitly, then let the visual settle.
    stage.stopSound(handle);
    yield* dot().to({ scale: 0.6, fill: '#e0664a' } as any, 0.5, easeOut('quad'));
    yield* wait(0.6);
});
