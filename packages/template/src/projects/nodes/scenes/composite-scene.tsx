 ...
 * own (no `renderSelf`, no `Graphics`). Instead it **builds its own regular node
 * children right in the constructor** via {@link Node.add}: given a list of
 ...
 * {@link Node.resolveContext} to apply those values via refs.)
 */
class BadgeGroup extends Node<BadgeGroupProps> {
    ...
    // `row` — that collides with NodeProps' grid-placement `row: number`.)
 ...
 * `BadgeGroup` is a user-defined {@link Node} subclass that draws nothing itself;
