import { Fill, sequence, easeInOutQuad, linear } from "@motion-script/core";
...
export class DashStrokeScene extends StrokeCardScene {
    ...
    *build() {
        ...
        const sample = this.card({
            stroke: { weight: 4, fill: Fill.color('#6990DD'), dash: [6, 10], dashOffset: 0, align: 'center' },
        });

        yield* sequence(
            // weight: hairline -> thick band.
            sample().strokeTo({ weight: 28, fill: Fill.color('#6990DD'), dash: [6, 10], align: 'center' }, 1.2, { ease: easeInOutQuad }),
            // dash pattern: tight dots -> long dashes / wide gaps (snaps at midpoint).
            sample().strokeTo({ weight: 28, fill: Fill.color('#E8617C'), dash: [60, 40], align: 'center' }, 1.2, { ease: easeInOutQuad }),
            // dashOffset: crawl the dashes one full pattern length (60 + 40) around the edge.
            sample().strokeTo({ weight: 28, fill: Fill.color('#E8617C'), dash: [60, 40], dashOffset: 100, align: 'center' }, 1.6, { ease: linear }),
            // dashOffset: keep crawling for continuous, loop-like motion.
            sample().strokeTo({ weight: 28, fill: Fill.color('#E8617C'), dash: [60, 40], dashOffset: 200, align: 'center' }, 1.6, { ease: linear }),
        );
    }
}
