...
/** A custom node that draws a raw `Graphics2D` label instead of holding a `Text`.
...
class DrawnLabel extends Node2D<{ label: string }> {
            ...
            children: (
                ...
                <DefaultTextStyle fontFamily={"Pixelify Sans"} height={"fill"} fill={"primary"} fontSize={64}>
                    <Rect width={"fill"} height={"fill"} flow={"vertical"} gap={32} align={"topCenter"}>
                        ...
                        {/* Not a Text node at all — a raw Graphics2D text op, which
                            inherits the same family/size from the draw scope. */}
                        <DrawnLabel label={"drawn Graphics2D, same family"} width={"fill"} height={80} />
