...
motion-script@latest`, or any project with a `src/project.ts` and a `vite.config`
registering `@motion-script/vite-plugin`). `vite` is a peer dependency; the
scaffolded templates already declare it.
