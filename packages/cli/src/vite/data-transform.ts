...
// The `/csv` subpath, not the package root: this plugin runs in Node, and core's
// main entry is emitted for a bundler (extensionless relative specifiers), which
// Node's ESM loader rejects. `dist/util/csv.js` is dependency-free, so the
// subpath loads standalone — and the parser stays single-sourced in core rather
// than forked here.
import { parseCSV } from '@motion-script/core/csv';
