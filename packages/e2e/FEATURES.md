...
Stroke has its own `fill` which also carries a `blend` property — composites the stroke paint against the shape beneath.
...
Shadow `fill` also carries `blend` — composites the shadow color against the backdrop.
